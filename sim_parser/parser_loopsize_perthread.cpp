...
void print_result(){
  ...
  int itr = 0;
  //printf("Kernel: %d\n",kernelcount);
//  printf("Kernel: %s",kernelname.c_str());
  for(int i=0;i<SMnum;i++){
    ...
    if(realloopsize == 0.0){
      assert(oracleloopsize == 0);
//      printf("  Core %d improves nan\n",i);
    }else{
...
//      printf("  Core %d improves %.2f%\n",i,((double)realloopsize/(double)oracleloopsize-1.0) * 100.0);
      printf("%d,%.4f\n",i,((double)realloopsize/(double)oracleloopsize-1.0));
    }
...
int detect_new(char* input){
  ...
  string tempkn = string(input);

  print_result();
  reinit();
  kernelname = tempkn.substr(8,tempkn.size()-8);
  return 1;
}
