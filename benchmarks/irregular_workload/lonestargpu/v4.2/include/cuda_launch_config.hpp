...
//  const size_t maxThreadsPerSM = properties.maxThreadsPerMultiProcessor;  // 768, 1024, 1536, etc.
  const size_t maxThreadsPerSM = (properties.maxThreadsPerMultiProcessor == 0)?1536:properties.maxThreadsPerMultiProcessor; 
  const size_t maxBlocksPerSM  = (max_blocks_per_multiprocessor(properties) == 0)?8:max_blocks_per_multiprocessor(properties);
